...
rear
    ...
    def forward(self, x):
        ...
        feature_dim = x.dim() - 1
        rms = torch.sqrt(torch.mean(x * x, dim=feature_dim, keepdim=True) + 1e-6)
        # Normalize and scale
...
def activation_quant(x: Tensor):
    ...
    """
    feature_dim = x.dim() - 1
    scale = 127.0 / x.abs().max(dim=feature_dim, keepdim=True).values.clamp_(min=1e-5)
...
class GLU(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        tail_dim = proj_out.dim() - 1
        chunks = proj_out.shape[tail_dim] // 2
...
class BitFeedForward(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if zero_init_output:
            init_zero_(self.ff[len(self.ff) - 1])
...
def rearrange_qkv(qkv: Tensor, num_heads: int) -> Tensor:
    ...
    head_dim = qkv.size(qkv.dim() - 1) // num_heads
...
def gqa_attention(query: Tensor, key: Tensor, value: Tensor, query_heads: int, kv_heads: int) -> Tensor:
    ...
    feature_dim = query.dim() - 1
    head_dim = query.size(feature_dim) // query_heads
    ...
    scale = head_dim ** -0.5
    attn = torch.matmul(query * scale, key.transpose(2, 3))  # [b, h, s, s]
    attn = F.softmax(attn, dim=attn.dim() - 1)
...
def scaled_dot_product_gqa(
    ...
    if scale is None:
        feature_dim = query.dim() - 1
        scale = query.size(feature_dim) ** 0.5
    query = query / scale
    ...
    attention = F.softmax(similarity / scale, dim=similarity.dim() - 1)
    if dropout > 0.0:
